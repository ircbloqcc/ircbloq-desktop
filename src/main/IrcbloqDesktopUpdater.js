...
import parseReleaseMessage from 'ircbloq-parse-release-message';
import {UPDATE_TARGET, UPDATE_MODAL_STATE} from 'ircbloq-gui/src/lib/update-state.js';
import {AbortController} from 'node-abort-controller';
class IrcbloqDesktopUpdater {
    constructor (webContents, resourceServer) {
        ...
        autoUpdater.setFeedURL({
                ...
                updaterCacheDirName: 'ircbloq-desktop-updater'
            });
    ...
    reqeustCheckUpdate () {
        ...
        if (app.getLocaleCountryCode() !== 'CN') { // eslint-disable-line no-negated-condition
            ...
            dialog.showMessageBox({
                ...
                message: `???????????????????,???????????????????????????????????????????: https://ircbloqcc.github.io/install-desktop-version` // eslint-disable-line max-len
            });
        }
    }
...
export default IrcbloqDesktopUpdater;
